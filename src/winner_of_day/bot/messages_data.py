...
def get_title_prompt(winner: str, winner_msg: str):
    if winner_msg == DEFAULT_WINNER_MSG:
        ...
        winner_msg_prompt = (
            f'Вот случайное сообщение этого пользователя: "{winner_msg}". Можешь учесть это сообщение в титуле.'
        )
    return f"""
Сгенерируй титул дня для пользователя чата. Это должен быть смешной и ироничный титул. Вот примеры таких титулов: Красавчик дня, Чудо дня, Cупер кекс, Населяющая сексмашина. Разрешено писать нецензурные выражения. Придумай оригинальный и смешной титул. 
В ответ присылай только титул дня, без лишних слов! Титул состоит из одного или двух слов. В титуле не должны упоминаться имя или фамилия пользователя. В ответе не должно быть кавычек. В титуле не должно быть орфографических ошибок.
